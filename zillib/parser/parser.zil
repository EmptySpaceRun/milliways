...
<DEFINE UPDATE-STATUS-LINE ()
	 ...
	 <COND (<AND ,SCOREMAD
			...
			<TELL ;,SCORE "  ^[rJ&" N ,HERE ,MOVES ,SCORE> ;Glitchy-effect)
		   (ELSE
			<TELL N ,SCORE>)>
	 ;<TELL N ,SCORE>
	 <TELL "    Moves: ">
...
<DEFINE SCORE-COUNT ("AUX" (NUM 0) (SEED 0) (FINAL 0) (SMTH 0))
	<COND (<AND ,SCOREMAD
		   		<NOT <EQUAL? ,SCOREOUT 32>>>
		   <COND (<G? ,SCOREOUT 10>
				  <SET SMTH <+ .SMTH 1>>)>
		   ...
		   <COND (,LIFE-PAIN
		   		  <SET SEED <+ .SEED 3>>)>
		   <REPEAT ()
		   		<COND (<EQUAL? .NUM ,SCOREOUT>
					   <SET FINAL <* .SEED 10>>
					   <RETURN>)>
				<SET NUM <+ .NUM 1>>
		   		<SET SEED <+ .SEED <+ 10 .SMTH>>>>
		   <SET NUM <- 5000 .FINAL ;.SEED>>
		   ;<TELL N .SEED>
		   .NUM)
		  (ELSE
		   <UPDATE-STATUS-LINE>)>>
...
<DEFINE MAIN-LOOP-1 ACT ("AUX" ICNT OCNT NUM (OBJ <>) V OBJ1 (NP <>) NP1 XX)
   <COND (<SETG P-WON <PARSER>>
	 ...
	 (T
	  <SETG CLOCK-WAIT T>
	  <SETG P-CONT <>>
      <COND (<AND <G? <GETB ,P-LEXV ,P-LEXWORDS> 0>
                  <L? <GETB ,P-LEXV ,P-LEXWORDS> 4>
...
<ROUTINE SAVE-INPUT (TBL "AUX" (OFFS 0) CNT TMP)
	 <SET CNT <+ <GETB ,P-LEXV <SET TMP <* 4 ,TLEXV>>>
		     <GETB ,P-LEXV <+ .TMP 1>>>>
