...
<DEFINE FIND-A-WINNER ACT ("OPT" (RM ,HERE))
 <COND (<AND <FSET? ,PRSO ,PERSONBIT>
    ...
    ,QCONTEXT)
       (<SETG QCONTEXT <FIND-FLAG-HERE-NOT ,PERSONBIT ,MUNGBIT ,PLAYER>>
    <TELL-SAID-TO ,QCONTEXT>
    ,QCONTEXT)
       (T
...
<DEFINE QCONTEXT-GOOD? ("AUX" PER)
 <COND (<AND ,QCONTEXT
	         <FSET? ,QCONTEXT ,PERSONBIT>
	         <NOT <EQUAL? ,QCONTEXT ,PLAYER>>
	         <EQUAL? ,HERE <META-LOC ,QCONTEXT>>>
	    ;<TELL-SAID-TO ,QCONTEXT>
        <RETURN ,QCONTEXT>)
	   (ELSE
		<COND (<IN? ,PLAYER ,INTERVIEW-ROOM>
               <SET PER <PICK-ONE <PLTABLE PERCY BENJY>>>
               <TELL-SAID-TO .PER>
               <SETG QCONTEXT .PER>
               <RETURN ,QCONTEXT>)
              (<AND <IN? ,PLAYER ,DARK>
